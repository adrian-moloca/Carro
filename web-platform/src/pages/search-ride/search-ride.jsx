  ...
  return (
    <Container className={"Primary-container-style"}>
      ...
      { ridesData.rides.length > 0 ? 
        rides.currentData().map((ride)=> ride.interactions.length > ride.statuses.length || ride.interactions.length === 0 || ride.interactions.length === ride.statuses.length ? <GetRide key={ride.rideId} name={ride.name} image={ride.image} rate={ride.rate ? ride.rate : 0} id={ride.id} rideId={ride.rideId} departure={ride.departure} destination={ride.destination} departureAddress={ride.departureAddress} destinationaAddress={ride.destinationaAddress} departureDate={ride.departureDate} estimatedTime={ride.estimatedTime} transportType={ride.transportType} statuses={[]} interactions={ride.interactions.filter((el) => !ride.statuses.some(pack => pack.packageId === el.packageId))} ridesUpdate={()=>searchForRides()}/> : null) : notFoundAnyRide()}
      </Grid>
