...
export const googleLogin = (token) => {
    return (dispatch) => {
        dispatch(fetchLoginRequest());
        axios.post(data.baseUrl + "/identity/auth/google", {
            accessToken: token
        })
        .then(response => {
            console.log(jwt_decode(response.data.token))
            getProfileStatus(response.data.token)
            dispatch(googleLoginSucces({...jwt_decode(response.data.token),token: response.data.token,refreshToken: response.data.refreshToken}));
        }).catch(error => {
            const errorMsg = error;
            dispatch(fetchLoginFailure(errorMsg));
        })
